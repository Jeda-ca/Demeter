...
namespace GUI
{
    public partial class Frm_GVendorsAdmin : Form
    {
        ...
        private void Frm_GVendedoresAdmin_Load(object sender, EventArgs e) // Evento Load del formulario.
        {
            //cbx_Buscar.SelectedIndex = 0; // Seleccionar la opción por defecto en el ComboBox de búsqueda.
            LoadVendorsData(); // Lógica para cargar datos de vendedores en el DataGridView.
        }

        //EVENTOS
        private void ibtn_Add_Click(object sender, EventArgs e) // Evento Click del botón "Agregar" para abrir Frm_Add
        {
            AddNewVendor(); // Llama al método para abrir el formulario de agregar vendedor.
        }
        private void ibtn_Modify_Click(object sender, EventArgs e) // Evento Click del botón "Modificar" para abrir Frm_ModifyV
        {
            if (dgv_ListaVendedores.SelectedRows.Count > 0) // Se verifica si hay un vendedor seleccionado en el DataGridView
            {
                ModifyVendorInfo(); // Llama al método para abrir el formulario de modificar vendedor.
            }
            else
            {
        ...
        private void ibtn_Delete_Click(object sender, EventArgs e) // Evento Click del botón "Eliminar" (placeholder para la lógica de eliminar vendedor)
        {
            if (dgv_ListaVendedores.SelectedRows.Count > 0) // Se verifica si hay un vendedor seleccionado en el DataGridView
            {
                DeleteVendor(); // Llama al método para eliminar el vendedor seleccionado.
            }
            else
            {
        ...
        {
            BuscarVendedor(); // Llama al método para buscar un vendedor basado en el texto y el criterio de búsqueda.
            
        }

        // MÉTODOS PRIVADOS
        private void LoadVendorsData() // Método placeholder para cargar/recargar datos de vendedores.
        {
        ...
        {
            Frm_AddVendorAdmin frmAdd = new Frm_AddVendorAdmin();
        ...
        private void ModifyVendorInfo()
        {
            ...
            Frm_ModifyVendorAdmin frmModifyV = new Frm_ModifyVendorAdmin(); // Si necesitas pasar datos, modifica el constructor de Frm_ModifyV
